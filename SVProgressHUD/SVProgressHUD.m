...
static const CGFloat SVProgressHUDRingRadius = 34;
...
-(void) updatePosition {
	...
	if (string) {
		...
		CGFloat labelRectY = imageUsed ? 73 : 9;
...
//	[self cancelRingLayerAnimation];
	...
	if (![self.class isVisible])
		[self.class showProgress:-1 status:string maskType:SVProgressHUDMaskTypeNone];
	...
	self.stringLabel.text = string;
	[self updatePosition];
	[self.indefiniteAnimatedView.superview bringSubviewToFront:self.indefiniteAnimatedView];
...
-(UILabel*) stringLabel {
	if (_stringLabel == nil) {
		...
		_stringLabel.textColor = SVProgressHUDRingColor;
